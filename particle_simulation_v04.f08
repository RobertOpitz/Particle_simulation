...
program particle_simulation
  ...
  call get_parameters(environment_file_unit, r_min, r_cut, &
                      ...
                      dr_max_tolerable, delta_t, write_time_step, max_time_step)
  ...
  max_nb_of_particles = size(particle_array)

  ! create cell_space space, and ill it with the particle_numbers
  write(*,*) "[INFO] initialize cell space"
  cell_space = initialze_cell_space(environment_file_unit, particle_array)
  ...
  call update_neighbor_list(particle_array, cell_space)
  ...
  call compute_forces(particle_array, active_particles)
  ...
  time_integration: do time_step = 1, max_time_step
    ...
    call compute_locations(particle_array, active_particles, displacement_flag)
    ...
    if (displacement_flag .eqv. .true.) then
      call update_cell_space(cell_space, particle_array)
      call update_neighbor_list(particle_array, cell_space)
      active_particles = update_active_particles(pack(cell_space, .true.), &
                                                 max_nb_of_particles)
    ...
    call compute_velocities(particle_array, active_particles)

    ! if it is time to write data, write data
    if (write_data_now(time_step)) then
      call write_data(output_file_unit, particle_array, active_particles)
    end if
  ...
  write(*,*) "[INFO] clean up"
  ! clean up
  call clean_up(particle_array, active_particles, cell_space)
...
contains
  ...
  subroutine get_parameters(environment_file_unit, r_min, r_cut, &
     ...
     read(environment_file_unit, *) write_time_step, max_time_step
  ...
  function initialze_cell_space(environment_file_unit, p) result(cell_space)
    integer, intent(in) :: environment_file_unit
    ...
    do i = 1, size(p)
      ...
      if (any(p(i)%location > length_of_area) .or. &
          ...
          nc = ceiling(p(i)%location / length_of_cells)
  ...
  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   subroutine compute_forces(p, active_particles)
     ...
     do m = 1, size(active_particles)
       i = active_particles(m)
       if (p(i)%nb_of_neighbors > 0) then
         do n = 1, p(i)%nb_of_neighbors
           j = p(i)%neighbor_list(n)
  ...
  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   subroutine compute_locations(p, active_particles, displacement_flag)
     type(particle_type), dimension(:), intent(in out) :: p !particle_array
     integer, dimension(:), intent(in) :: active_particles
  ...
  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   subroutine compute_velocities(p, active_particles)
     type(particle_type), dimension(:), intent(in out) :: p ! particle_array
     integer, dimension(:), intent(in) :: active_particles
  ...
  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   subroutine write_data(ofu, p_array, active_particles)
     ...
     do n = 1, m
       ...
       d = d + 1
       write(unit=ofu, rec=d) p_array(i)%id

       ! write location of particle
       do j = 1, 3
         d = d + 1
         write(unit=ofu, rec=d) p_array(i)%location(j)
       end do

       ! write velocity of particle
       do j = 1, 3
         d = d + 1
         write(unit=ofu, rec=d) p_array(i)%velocity(j)
       end do
     end do
  ...
  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   subroutine clean_up(p_array, active_particles, cell_space)
     type(particle_type), dimension(:), allocatable, intent(in out) :: p_array
