...
contains
  ...
  function initialize_particle_array(particle_file_unit) result(particle_array)
    ...
    do i = 1, nb_of_particles
       ! read particle data
       read(particle_file_unit, *) id, location, velocity
  ...
  subroutine update_neighbor_list(particle_array, linked_list)
  ...
  !
   ...
   subroutine compute_forces(particle_array)
     type(particle_type), dimension(:), intent(in out) :: particle_array

     integer :: i, j

     do i = 1, size(particle_array)
       do j = 1, particle_array(i)%nb_of_neighbors
         call compute_force_between_particles(particle_array(i), &
                                              particle_array(j))
       end do
     end do
