...
def get_data(filePath, display, nb_of_bytes = 4):
    ...
    with open(filePath,"rb") as f:
        ...
        nb_of_entries = read(f, integer_format_descriptor)
        print("Number of blocks: " + str(nb_of_entries))

        data = []
        block_counter = 0
        while True:
            # nb records of second block
            nb_of_entries_in_block = read(f, integer_format_descriptor)
            if nb_of_entries_in_block is None:
                break
            
            block_counter += 1
            print(str(block_counter) + ". block: " + str(nb_of_entries_in_block))

            pos = []
            for _ in range(int(nb_of_entries_in_block/7)):
