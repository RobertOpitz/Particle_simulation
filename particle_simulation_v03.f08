...
program particle_simulation
  ...
  !active_particles = update_active_particle_list(unpack(cell_space, .true.), &
  !                                               size(particle_array))
  ...
  time_integration: do time_step = 1, max_time_step
    ...
    call compute_locations(particle_array, displacement_flag) !active_particles)
    ...
    if (displacement_flag .eqv. .true.) then
      ...
      call update_neighbor_list(particle_array, cell_space)
      !active_particles = update_active_particle_numbers(unpack(cell_space, .true.), &
      !                                               size(particle_array))
    end if
  ...
  ! clean up
  call clean_up(particle_array, cell_space)!, active_particles)
...
contains
  ...
  function initialze_cell_space(environment_file_unit, &
  ...
  subroutine entry_in_new_list(particle, new_cell, linked_cells)
     ...
     if (any(new_cell < (/1,1,1/)) .or. any(new_cell > max_cell)) then
       ...
       !call delete_particle_number(particle_number)
  ...
  ! function update_active_particle_numbers(cell_space, &
  !                               nb_of_particles) result(active_particle_numbers)
  !   type(cell_type), dimension(:), intent(in) :: cell_space
  !   logical, dimension(nb_of_particles) :: active_particles
  !   integer, dimension(:), allocatable :: active_particle_numbers
  !
  ...
  !       active_particles(number%i) = .true.
  !       n = n + 1
  !       number => number%next_number
  !     end do
  !   end do
  !
  !   allocate(active_particle_numbers(n))
  !
  !   j = 0
  !   do i = 1, nb_of_particles
  !     if (active_particle .eqv. true) then
  !       j = j + 1
  !       active_particle_numbers(j) = i
  !     end if
  !   end do
  !
  ! end function update_active_particle_numbers
  ...
  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   subroutine compute_locations(p, displacement_flag)
     ...
     do i = 1, size(p)
     !do i = 1, size(active_particles)
       !if (.not. p(i)%is_active) cycle
  ...
  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   subroutine compute_velocities(p)
     ...
     !do i = 1, size(active_particles)
        !j = active_particles(i)
        !p(j)%velocity = p(j)%velocity + time_mass_factor * &
        !                (p(j)%force_new + p(j)%force_old)

       !if (p(i)is_active) then
  ...
  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   subroutine write_data(ofu, p_array, linked_cells)
     ...
     all_cells: do ci = 1, size(linked_cells)
       ...
       p => linked_cells(ci)%first_number
       this_cell: do while(associated(p))
         !if (p_array(p%i)%is_active) then
         ! write particle id
         d = d + 1
